...
<head>
	...
	<style>
		body{font-family:system-ui,Segoe UI,Roboto,Arial;margin:12px;background:#f7f7fb;color:#111}
		#ui{display:flex;gap:12px;align-items:center;margin-bottom:8px}
		canvas{background:#79a7ff;border-radius:8px;display:block}
...
<body>
	...
	<script>
	// Minimal OOP Whack-a-Mole implementation
	class Entity {
	...
	class Game{
		...
		handleClick(mx,my){ for(const hole of this.holes){ if(hole.entity){ const s = hole.size/2; if(mx>=hole.x-s && mx<=hole.x+s && my>=hole.y-s && my<=hole.y+s){ hole.entity.onHit(this); return; } } } // miss: penalty
		...
		update(dt){ // update entities
			for(const h of this.holes) if(h.entity) h.entity.update(dt,this);
			// spawn logic
			this.spawnTimer += dt; if(this.spawnTimer > this.spawnInterval){ this.spawnTimer =0; this.spawnEntity(); }
			// remove expired entities handled by entity.expire
		}
...
Notes:
- The HTML block above contains all necessary JS classes: `Game`, `Hole`, `Entity`, `Mole`, `PowerUp`, and `GoldenMole`.
- `GoldenMole` appears ~5% of the time and grants double points.
- High scores and last scores are stored in `localStorage` keys `whackHigh` and `whackLast`.

Open the fenced HTML as a file in your browser to play.
